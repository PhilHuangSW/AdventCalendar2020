...
const run = (instructions) => {
  ...
  while (idx < instructions.length) {
    let instruct = instructions[idx].slice(0, 3)
    ...
    switch (instruct) {
      case 'nop':
        ...
        break
      case 'acc':
        let accNum = parseInt(instructions[idx].slice(4, instructions[idx].length))
        console.log(`accNum: ${accNum}`)
        accumulator += accNum
        idx += 1
        break
      case 'jmp':
        let jmpNum = parseInt(instructions[idx].slice(4, instructions[idx].length))
        console.log(`jmpNum: ${jmpNum}`)
        idx += jmpNum
        break
      default:
        idx += 1
